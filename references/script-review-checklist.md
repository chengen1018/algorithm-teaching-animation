...
- 帶有證據的 findings
- 修復方向：`SCRIPT`、`COLLECT_REQUIREMENTS` 或 `DESIGN_DEVELOPMENT`
...
- script 是否留在已確認需求與已核准設計的範圍內，而沒有發明新語意？
- beats 是否遵守已核准動畫設計與畫面附加資訊規則？
- 是否有任何措辭把本該在需求或設計中決定的分歧藏起來，改由 script 自行猜測？

若 script 暴露使用者需求記錄不準確，則回到 `COLLECT_REQUIREMENTS` 修正後重新送入設計流程。
若 script 之所以能繼續，是因為已核准設計本身在演算法語意、主要心智模型、核心視覺語意、場景結構、資訊層級、教學弧線、高層節拍或使用者選定設計上有缺漏或衝突，則回到 `DESIGN_DEVELOPMENT`。必須先設計修復、重新審查與重新核准。
...
若已核准設計已有正確教學邏輯，但 script 解釋太弱或順序錯誤，則修復留在 `SCRIPT`。

### Beat Completeness

- 每個 beat 是否都定義了 viewer goal、algorithm moment、visual focus、teaching note、progress cue 與 voiceover intent？
- 當設計說 support structures、pointers、boundaries 與 temporary slots 很重要時，它們是否被清楚命名？
...
若上游來源清楚，但某個 beat 太粗，不足以支撐忠實同步 narration，則修復留在 `SCRIPT`。
...
若 voiceover readiness 失敗的原因是使用者需求記錄不準確，則回到 `COLLECT_REQUIREMENTS`。
若 voiceover readiness 失敗的原因是已核准設計缺少或衝突了必要教學決策，則回到 `DESIGN_DEVELOPMENT`。必須先設計修復、重新審查與重新核准。

## 修復路由

### 留在 SCRIPT

當已確認需求與已核准設計清楚，而問題只限於：
...
當失敗暴露使用者需求遺漏、來源擷取不準確或語言記錄錯誤時，使用此路徑。修正後必須重新送入設計流程。
...
- 本該升級處理的需求或設計歧義，卻在 script 內被偷偷補掉

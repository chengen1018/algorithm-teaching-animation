...
The user must explicitly approve the current reviewed version of `animation_design.md`. The user may edit `animation_design.md`, but every edit invalidates prior review and requires re-review before approval can be accepted. Silence, inactivity, or editing the file alone does not count as approval.

Approval applies only to the exact reviewed version. Any later material change requires review and explicit confirmation again.
...
- [ ] Structure presentation and information hierarchy are defined — evidence: ...
